        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <li class="with-x" v-if="searchParams.categoryName">
              {{ searchParams.categoryName }}<i @click="removeParams">×</i>
            </li>
...
export default {
  ...
  mounted() {
    this.getSearchInfo();
    this.$bus.$on('initPageNo', this.initPageNo); // 重置pageNo
  },
  beforeDestroy(){
    this.$bus.$off('initPageNo');
  },
  methods: {
    // 请求数据
    getSearchInfo() {
    ...
    searchForProps(attrValue, attr) {
      let prop = `${attr.attrId}:${attrValue}:${attr.attrName}`; // 拼接参数
      // some => 只要有一个满足就返回true，参数是回调函数
      // every => 所有都满足就返回true，参数是回调函数
      let isRepeat = this.searchParams.props.some((item) => item === prop); // 判断是否含有该参数
      if (isRepeat) {
        // 如果为true，直接结束函数
        return;
      }
    ...
    changeSort(sortFlag) {
      let newOrder = "";
      // 判断点击的是不是选中的，如果是，更改排序类型，不是，就更换排序标志
      if (sortFlag === this.sortFlag) {
        // 是选中的排序标志，更改排序类型
        newOrder = `${sortFlag}:${this.sortType === "desc" ? "asc" : "desc"}`;
      } else {
        // 不是选中的排序标志，更改排序标志，然后默认排序类型desc
        newOrder = `${sortFlag}:desc`;
      }
      this.searchParams.order = newOrder; // 更新数据
