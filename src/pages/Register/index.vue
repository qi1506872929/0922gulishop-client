    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <input
          ...
          v-model="password"
          name="password"
          v-validate="{ required: true, regex: /^[0-9 A-z]{6,16}$/ }"
      ...
      <div class="controls">
        <input
          ...
          :class="{ required: true, invalid: errors.has('isCheck') }"
        />
    ...
    async getCode() {
      if (this.phone) {
        try {
          ...
          alert("请求验证失败" + error.message);
        }
      }
    },
    // 注册事件
    async userRegister() {
      const success = await this.$validator.validateAll(); //对所有表单项进行验证，验证通过返回 true，否则返回 false
      if (success) {
