...
export default {
  ...
  methods: {
    ...
    async pay() {
      try {
        let imgUrl = await QRCode.toDataURL(this.payInfo.codeUrl); // 获取二维码图片
        // 弹框
        this.$alert(`<img src="${imgUrl}"/>`, "请使用微信扫码支付", {
          ...
          beforeClose: (action, instance, done) => {
            // action 的值为'confirm'确定, 'cancel'取消或'close'关闭
            if (action === "confirm") {
              if (!this.payStatus) {
                // this.$message.warning("请确保支付成功，成功会自动跳转支付成功页面");
                // 下面是直接跳转页面，方便操作
                clearInterval(this.timer); // 清除定时器
                this.timer = null;
                done(); // 调用就关闭弹框
                this.$router.push("/paysuccess"); // 跳转页面
              }
            } else if (action === "cancel") {
              this.$message.warning("支付遇到问题请咨询尚硅谷前台小姐姐");
          ...
          .then(() => {})
          .catch(() => {}); // 点击确定和取消之后的事件，但是会自动关闭弹框，所以不使用，但是不写点取消会报错
        if (!this.timer) {
          this.timer = setInterval(async () => {
            let result = await this.$API.reqPayStatus(this.orderNum);
            if (result.code === 200) {
              this.payStatus = 200; // 存储状态用来判断点击确定的后续操作
              this.$message.success("支付成功，即将跳转支付成功页面"); // 提示信息
              clearInterval(this.timer); // 清除定时器
              this.timer = null; // 重置定时器的数据，否则无法再进入判断
        ...
        this.$message.error("生成二维码链接失败" + error.message);
      }
    },
  },
};
