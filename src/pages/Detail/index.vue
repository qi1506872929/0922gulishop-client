                ...
                <input
                  ...
                  @change="
                    $event.target.value > 1
                      ? (skuNum = Math.ceil($event.target.value))
                      : (skuNum = 1)
                  "
                />
...
export default {
  ...
  methods: {
    ...
    changeChecked(spuSaleAttrValueList, spuSaleAttrValue) {
      // spuSaleAttrValueList当前属性的所有属性列表 spuSaleAttrValue 代表点击的属性值
      spuSaleAttrValueList.forEach((item) => (item.isChecked = "0")); // 所有属性都为未选中
      spuSaleAttrValue.isChecked = "1"; // 将点击的改为选中
    },
    // 添加购物车
